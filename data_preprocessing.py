...
def Import():
    ...
    data.loc[data['Glucose']==0.0]=data_mean

    #处理BloodPressure
    data_mean1=data['BloodPressure'].mean()
    data.loc[data['BloodPressure']==0.0]=data_mean1

    """ #处理SkinThickness
    data_mean2=data['SkinThickness'].mean()
    data.loc[data['SkinThickness']==0.0]=data_mean2 """ 

    """ #处理Insulin
    data_mean3=data['Insulin'].mean()
    data.loc[data['Insulin']==0.0]=data_mean3 """          ##在处理这两个特征时发现会篡改其他特征下的值以及目标分类下的特征值，目前没有找到较好的办法
    ...
    data.loc[data['DiabetesPedigreeFunction']==0.0]=data_mean4
...
def LowVarianceFiltering(data):
    transfer=VarianceThreshold(threshold=200)
    data_new=transfer.fit_transform(data)
...
if __name__=="__main__":
    data,target=Import()       ##对某些特征下的0值做了处理，用特征均值取代，保证较多的可用数据
    
    print("数据归一化：")
    data=Normalization(data)  ##在只进行归一化后，通过决策树得到的准确率变化不大，都在0.69~0.72的范围内，使用随机森林准确率在0.75~0.77
    ...
    print("数据标准化：")
    data=Standardization(data) ##在只进行标准化后，通过决策树得到的准确率在0.71~0.75之间，使用随机森林准确率为0.71~0.78
    DecisionTree(data,target) 
    RandomForest(data,target)
    ...
    data=LowVarianceFiltering(data)
    DecisionTree(data,target) 
    RandomForest(data,target)
